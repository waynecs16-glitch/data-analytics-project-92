...
-- (общая сумма продаж, делённая на количество совершённых им продаж).
-- Затем он сравнивает эту индивидуальную среднюю выручку со средней выручкой
-- за сделку по всем продажам в базе данных.
...
GROUP BY
    ...
    seller
HAVING
    (SUM(s.quantity * p.price) / COUNT(s.sales_id)) < (
        SELECT
            AVG(sq.quantity * pr.price)
        FROM
            sales AS sq
        JOIN
            products AS pr ON sq.product_id = pr.product_id
    )
ORDER BY
    average_income ASC;

-- Этот запрос рассчитывает общую выручку (количество проданного товара умноженное на его цену)
-- для каждого сотрудника по каждому дню недели.
-- Выручка округляется до целого числа, и результаты сортируются сначала по числовому значению дня недели,
-- а затем по имени продавца.
SELECT
    e.first_name || ' ' || e.last_name AS seller,
...
-- Этот отчёт группирует данные по месяцам (в формате "ГГГГ-ММ") и подсчитывает количество
-- уникальных покупателей и общую выручку за каждый месяц.
...
-- Этот запрос находит покупателей, чья самая ранняя покупка была совершена с ценой товара,
-- равной 0 (что указывает на акцию). Для каждого такого покупателя будут отображены его имя и фамилия,
-- дата первой акционной покупки и имя продавца, совершившего эту сделку.
